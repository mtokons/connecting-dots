...
import { 
  ...
  TrendingUp, Zap, AlertCircle, Radio, Eye, Sparkles
} from 'lucide-react';
...
const TABS = [
  { 
    ...
    description: 'Verified EC Results',
  ...
  { 
    ...
    description: 'News Portal Aggregation',
    source: 'Daily Star, Prothom Alo, bdnews24'
  },
  { 
    id: 'prediction', 
    label: 'AI Prediction', 
    ...
    description: 'TensorFlow.js Model',
    source: 'Bayesian + Momentum Analysis'
  },
];
...
export default function Home() {
  ...
  // Update refresh time when SSE pushes new data
  useEffect(() => {
    if (lastUpdate) {
  ...
  const getViewData = (viewId) => {
    ...
    switch (viewId) {
      case 'official':
        // Official EC data - only verified/declared seats
        return parties.map(party => ({
          ...party,
          color: '#22c55e', // Green tint for official
          displaySeats: party.seats?.filter(s => s.status === 'declared').length || 0,
          totalSeats: party.totalSeats,
          isVerified: true,
        })).filter(p => p.displaySeats > 0);
      
      case 'unofficial':
        // Unofficial - all counting + declared from news sources
        return parties.map(party => ({
          ...party,
          color: '#f97316', // Orange for unofficial
      ...
      case 'prediction':
        ...
        return predictions.map(pred => ({
          ...
          color: '#3b82f6', // Blue for predictions
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Three views: Official EC results, News aggregation, AI predictions
          </p>
        </div>
...
function SummaryStats({ data, viewType, dashboard, tabConfig }) {
  ...
  const stats = viewType === 'prediction' 
    ? [
        { label: 'Predicted Leader', value: leader?.short_name || '-', color: tabConfig.color },
    ...
    : [
        { label: viewType === 'official' ? 'Declared' : 'Reporting', value: totalSeats, color: tabConfig.color },
...
function D3TreeView({ data, viewType, color }) {
  ...
  useEffect(() => {
    ...
    // Add glow filter for predictions
    ...
    nodes.filter(d => d.depth === 1)
      .append('circle')
      .attr('r', d => Math.max(15, Math.min(40, (d.data.value || 0) / 4)))
      .attr('fill', color + '20')
      .attr('stroke', color)
      .attr('stroke-width', 2)
      .attr('filter', viewType === 'prediction' ? 'url(#glow)' : 'none');
    ...
    nodes.filter(d => d.depth === 1)
      .append('text')
      .attr('text-anchor', 'middle')
      .attr('dy', d => Math.max(20, Math.min(45, (d.data.value || 0) / 4)) + 15)
      .attr('fill', color)
      .attr('font-weight', 'bold')
      .attr('font-size', '14px')
...
// ─── PARTY CARDS GRID ───────────────────────────────────
function PartyCardsGrid({ data, viewType, tabConfig }) {
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
      {data.slice(0, 10).map((party, i) => (
        <div 
          key={party.id} 
          className={`glass-card-hover p-4 ${tabConfig.bgColor} border ${tabConfig.borderColor}`}
        >
          ...
          <div className="text-3xl font-black" style={{ color: tabConfig.color }}>
            {party.displaySeats}
          </div>
          <div className="text-xs text-slate-500 mt-1">
            {viewType === 'prediction' ? 'predicted' : viewType === 'official' ? 'declared' : 'leading'}
          </div>
          {viewType === 'prediction' && party.winProbability && (
            <div className="mt-2 text-xs">
              ...
              <div className="h-1 bg-white/10 rounded-full mt-1 overflow-hidden">
                <div 
                  ...
                  style={{ 
                    width: `${party.winProbability * 100}%`,
          ...
          {viewType === 'unofficial' && party.votePercentage && (
            <div className="text-xs text-slate-400 mt-1">
              {party.votePercentage.toFixed(1)}% vote share
            </div>
          )}
